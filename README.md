...
時点で反映されていたことを表す。Squareとの最終同期日時は、商品一覧では状態バッジの下、
...
差分がない場合もSquareとの最終同期日時は更新するが、`items.updated_at`は変更しない。
...
- 商品情報と写真の反映状況を「Square未反映／Square反映済み」など4状態とSquareとの最終同期日時で表示する
