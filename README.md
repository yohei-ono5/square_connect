...
そのまま表示・編集する。
商品詳細の説明文タブでは、商品名・サイズ・採寸・コンディションから生成した本文を
「自動で作成（Squareへの登録・更新時に反映されます）」として表示する。商品番号（SKU）は説明文に含めない。
「本文をコピー」を押すと全文をクリップボードへコピーでき、
「Squareに登録」または「Squareを更新」を押すと、同じ本文をSquareの商品説明欄へ反映する。
...
未実装である。自動取得が一時的に失敗した場合は、最後に取得できた商品情報と最終確認日時を
維持し、エンドユーザーにはエラーを表示しない。手動更新が失敗した場合はエラーコードを表示する。
...
差分がない場合もSquareの最終確認日時は更新するが、`items.updated_at`は変更しない。
Squareから取得した商品情報に差分がある場合だけ`items.updated_at`を更新するため、商品詳細を
閲覧しただけでは商品一覧の更新日時順に影響しない。
Square登録済み商品の同期済み在庫数が0の場合は、商品一覧の商品名横に`売切`バッジを表示する。
在庫数が1以上へ戻った場合は、次回のSquare同期後にバッジを非表示にする。
...
- 商品情報と写真の反映状況を「Square未反映／Square反映済み」など4状態とSquareの最終確認日時で表示する
