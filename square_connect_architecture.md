...
- **DB — Supabase（Postgres / ストレージ）**
  Webアプリの段階から導入。テスト運用中はSupabase Authを使わず、anonキーで店舗・商品・写真を読み書きする。本運用時にログインと店舗単位のRLSを導入する。APIロジックは持たず、データストアとしての役割に限定。
...
│ (将来RNへ移行)│   │ メルカリ連携   │   │  Auth = テスト中は未使用 │   │  → 最終確認      │
│             │   │ SKU重複チェック│   │                       │→ │  → 公開          │
└────────────┘   └──────────────┘   └──────────────────────┘   └────────────────┘
```
...
### Phase 1 の作業ブロック
1. **最小商品登録（最優先）**：必須入力を「管理番号（SKU、手入力）」「商品名」「金額」の3項目のみに絞り、これだけでSquareへ非公開作成できるルートを最初に通す。対象・カテゴリ・サイズ・写真・採寸4項目・コンディションはすべて未入力でよい。
...
4. **後追い編集**：対象・カテゴリ・サイズ・写真・採寸・コンディションは商品登録後いつでも追記・修正でき、更新時にSquare側の商品情報（タイトル・説明文含む）も反映する。
...
[クイック登録] 商品番号（SKU、手入力）・商品名・金額を入力
   │ 「下書き保存」（ローカル保存のみ）／「Squareに登録」（SKU重複チェック→Square非公開作成→保存）
   ▼
...
Supabase更新 → Square側の商品情報（タイトル・説明・画像）も更新
   │
   ▼
（任意タイミングで）[Squareに登録] ボタン → スタッフがSquareダッシュボード側で最終確認・公開（アプリ内に公開ボタンは置かない）
```
...
**Square登録済みの判定**：`items.square_object_id`が非nullかどうかで判定する。「Squareに登録」ボタンのAPI呼び出しが成功した時だけこの値がセットされる設計なので、追加の実装・通信なしにボタン処理の完了を判定できる。Square側で後から削除・アーカイブされた場合の乖離は検知できないため、それを検知したい場合はSquare APIへの再照会（`RetrieveCatalogObject`）や`catalog.version.updated`Webhook同期（[[square-mercari-api-capabilities]]参照）が必要——Supabase移行後の課題として残す。
3. **クイック登録** — 商品番号（SKU）・商品名・金額の3フィールド（**2026-07-16変更：SKUは自動採番ではなくスタッフの手入力**。入力欄はフォームの先頭に配置）。商品詳細編集画面と同じく「下書き保存」（Squareへは送らずローカル保存のみ）／「Squareに登録」（即Square非公開作成）を選べる。写真を今すぐ足せる余裕があるかどうかは店舗判断のため、写真添付も任意で用意。**登録後は一覧に戻さず商品詳細編集画面へ直接遷移**——別に「ちゃんと入力する用」のフォームを用意する代わりに、時間があればそのまま詳細タブで続けて入力でき、無ければ途中で離脱して一覧から後で戻ればよい。手入力SKUの重複は、保存前にアプリ側で商品一覧内をチェックし、Square登録時はWorkerが`SearchCatalogObjects`でSquare側の重複も別途チェックする（二重の防御）。
...
├── supabase/
│   └── migrations/            # items / item_photos / stores のSQL・公開RLSポリシー
...
- `apps/web`はSquare APIを直接叩かず、`apps/worker`が公開する自前のエンドポイント（例：`POST /api/items/:id/register-to-square`、**2026-07-20追加：`GET /api/square/categories`**——Squareの`ListCatalog`（`types=CATEGORY`、カーソルページネーション）を呼び出し、`category_data.parent_category`から親カテゴリ名を解決した`{id, name, parentName}`の配列を返す。トークンはWorker内に閉じたまま）を呼ぶだけ。
