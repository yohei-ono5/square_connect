...
- Supabaseの管理権限には新しいSecret key（`sb_secret_...`）を使用し、`SUPABASE_SECRET_KEY`としてCloudflare Workers Secretsへ登録する。Legacy API Keysの`service_role`は使用せず、Data APIには`apikey`ヘッダーだけで送信する。
