...
- **タブ（4つ、単一画面内で切り替え。2026-07-16改訂：並び順を基本情報→写真→採寸→説明文に変更。商品一覧から遷移した際の初期表示タブも基本情報に統一）**
  - **基本情報**：画面内を「Square連携項目」と「アプリ管理項目」に分ける。Square連携項目は商品番号（SKU）・商品名・カテゴリ・価格。他の商品と重複するSKUに変更すると赤字で警告表示する。カテゴリはSquareの`ListCatalog` APIから取得した既存カテゴリを「大カテゴリ→中カテゴリ」の2段階で選択し、自由入力は行わない。取得結果はセッション中1回だけWorker経由でフェッチしキャッシュする。アプリ管理項目は対象（メンズ／レディース／ユニセックスのセレクト）、表記サイズ、コンディション。表記サイズはXXS〜XXXL・FREEのプルダウンと「その他」の自由入力を用意し、既存の任意表記も保持する。コンディションの先頭には「未設定（後で設定）」を用意する。表記サイズとコンディションはSquareの専用フィールドではなく商品説明文へ反映し、対象はSquareへ送信しない。
