...
| 連携 | Square Catalog API（商品・画像の登録／更新／画像削除）。SKU重複はAPI側で保証されないためアプリ側で事前チェック |
| 形態 | 1店舗 → SaaS |

> ステータス：**Phase 0 完了 / Phase 1実装・実環境検証中**（Supabase商品保存、R2写真保存、Square商品・画像連携まで実装済み）
> 最終更新：2026-07-22
...
items        (item_id, store_id→stores.store_id, status['draft'|'confirmed'|'pushed'],
              mgmt_no NOT NULL（スタッフの手入力、SKUとして利用。店舗内での重複はアプリ側でチェック。
                          数字のみの想定だが先頭ゼロ（例：01041）を保持するため型は文字列のまま、
                          入力欄もtype="number"にはしない。inputMode="numeric"でモバイルの数字キーパッドだけ促す）,
...
-- mgmt_no はスタッフの手入力（2026-07-16変更：自動採番から変更）。店舗内の重複はアプリ側で事前チェックし、
...
| **1** | 🚧 実装・検証中 | **Webアプリ化 ＋ Supabase ＋ R2 ＋ Square連携** — 商品・画像登録まで実装済み。同期状態の可視化は次の作業。 |
...
[クイック登録] 商品番号（SKU、手入力）・商品名・金額を入力
   ...
   ▼
[商品詳細編集へそのまま遷移]（一覧には戻らない＝時間に余裕があればそのまま続けて入力できる）
   │
...
**Square登録済みの判定**：`items.square_object_id`が非nullかどうかで判定する。Square側の変更は`catalog.version.updated` WebhookでSupabaseへ反映する。ただし、一覧上で「Squareの最新状態といつ照合したか」を示す同期状態・最終確認日時の表示は未実装であり、手動照合・定期照合とあわせて次の作業とする。
3. **クイック登録** — 商品番号（SKU）・商品名・金額の3フィールド（SKUはスタッフの手入力）。「下書き保存」を押した場合だけSupabaseの下書きとして一覧へ残す。「Squareに登録」では処理用の商品・写真を一時保存するが、Square登録が失敗した場合は完全破棄し、入力値と選択画像を保持したまま画面に留まる。成功時だけSquare IDをSupabaseへ保存し、商品詳細編集画面へ遷移する。Square作成後に応答だけ失われた場合はSKU再照会でIDを復旧し、重複登録を防ぐ。
...
- **ヘッダー**：戻る導線／商品名・SKU・価格（**2026-07-16改訂：「SKU」ラベルは付けず番号のみ表示**）／状態バッジ（Square登録済み／Square未登録）
...
- **フッター（常設）**：「下書き保存」（プライマリ・左／Supabase更新→Square側にも反映）／「Squareに登録」（セカンダリ・右／Squareダッシュボードで最終確認・公開する導線）。頻度の高い保存操作を主役にし、Square側の操作は控えめな見た目にする。
...
- `apps/web`はSquare APIを直接叩かず、`apps/worker`の`POST /api/items/:id/register-to-square`、`POST /api/items/:id/photos`、`DELETE /api/items/:id/photos/:photoId`、`POST /api/items/:id/photos/sync-to-square`、`GET /api/square/categories`を利用する。SquareトークンとR2バインディングはWorker内に閉じる。
