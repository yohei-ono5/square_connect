...
**Square登録済みの判定**：`items.square_object_id`が非nullかどうかで判定する。Square側の変更は`catalog.version.updated` WebhookでSupabaseへ反映する。表示中の画面は自動更新せず、商品詳細の「Squareの最新情報を取得」で対象IDの最新値を手動反映できる。一覧上で「Squareの最新状態といつ照合したか」を示す同期状態・最終確認日時の表示と定期照合は未実装である。
...
- **ヘッダー**：戻る導線／商品名・SKU・価格（**2026-07-16改訂：「SKU」ラベルは付けず番号のみ表示**）／状態バッジ（Square登録済み／Square未登録）／登録済み商品の「Squareの最新情報を取得」
