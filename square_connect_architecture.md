# square_connect — アーキテクチャ & ロードマップ

中古Tシャツの採寸・撮影から商品登録までを効率化し、Square Catalogへ商品と画像を登録する。
...
| DB | **Supabase**（Postgres / Auth）。画像本体はCloudflare R2 |
| サーバー実行環境 | **Cloudflare Workers** — Square/メルカリ連携・SKU重複チェック等のAPIロジックを実行。Squareトークンを秘匿 |
| 連携 | Square Catalog API（商品・画像の登録／更新／画像削除、商品情報の手動再取得）。SKU重複はAPI側で保証されないためアプリ側で事前チェック |
| 形態 | 1店舗 → SaaS |
...
- **DB — Supabase（Postgres）／画像 — Cloudflare R2**
  商品・店舗・写真メタデータはSupabaseへ、画像ファイル本体は非公開R2バケットへ保存する。テスト運用中はSupabase Authを使わず、anonキーで店舗・商品・写真メタデータを読み書きする。本運用時にログインと店舗単位のRLSを導入する。
...
items        (item_id, store_id→stores.store_id, status['draft'|'confirmed'|'pushed'],
              ...
              gender['mens'|'womens'|'unisex'], category, size  -- NULL可（後から追記可）,
              condition NULL可（既定値NULL＝「後で設定」）,
              m_shoulder, m_chest, m_length, m_sleeve  -- すべてNULL可（後から採寸・編集可）,
              description, square_object_id, square_variation_id, square_version,
              square_synced_at, square_deleted_at, updated_at, deleted_at)
...
-- Square同期（Service Roleを持つWorkerだけが操作）
...
-- gender / category / size / condition / 採寸4項目 / 写真は未設定のままでも Square へ登録・公開してよい（後から更新すれば Square 側にも反映）
...
[クイック登録] 商品番号（SKU、手入力）・商品名・金額を入力
   ...
   │   └─ 写真なし：画像同期を行わない／写真あり：同期失敗時だけ警告する
   ▼
[商品一覧へ遷移]（連続して次の商品を登録できる）
   │
   │ ← 一覧からもいつでも・何度でも商品をタップして再訪可能
   ▼
[商品詳細編集]
   ...
   │ 「下書き保存」＝Supabaseだけを更新
   │ 「Squareに登録／Squareを更新」＝Supabase保存後にSquareへ反映
   │ 「Squareの最新情報を取得」＝Square IDを直接指定して商品名・SKU・価格・説明文を取り込む
   ▼
選択した操作に応じてSupabaseのみ、またはSupabase＋Squareを更新
   │
   ▼
（任意タイミングで）[Squareに登録] ボタン → 商品と保存済み写真をSquareへ登録
```
...
**Square同期状態の判定**：`square_object_id`、`updated_at`、`square_synced_at`、`square_deleted_at`と、`item_photos.square_image_id`の有無から、`Square未登録`、`Square未反映`、`Square同期済み`、`Square側で削除済み`の4状態を算出する。登録済み商品を下書き保存した場合や未同期写真がある場合は`Square未反映`とし、Square更新または最新情報取得の成功後に`Square同期済み`へ戻す。Square側の変更は`catalog.version.updated` WebhookでSupabaseへ反映し、ブラウザへフォーカスが戻った際に一覧を自動再読込する。登録済みの商品詳細を開いた際は対象IDの最新値をSquareから自動取得し、手動取得ボタンも提供する。最終確認日時と同期エラー履歴の表示、定期照合は未実装である。
3. **クイック登録** — 商品番号（SKU）・商品名・金額の3フィールド（SKUはスタッフの手入力、金額は0〜9のみ入力可）。「下書き保存」を押した場合だけSupabaseの下書きとして一覧へ残す。「Squareに登録」では処理用の商品・写真を一時保存するが、Square登録が失敗した場合は完全破棄し、入力値と選択画像を保持したまま画面に留まる。成功時だけSquare IDをSupabaseへ保存し、商品一覧へ遷移する。写真なしでは画像同期を行わず、写真ありで同期に失敗した場合だけ警告する。Square作成後に応答だけ失われた場合はSKU再照会でIDを復旧し、重複登録を防ぐ。
4. **商品詳細編集**（タブ構成、いつでも中断・再開可。**2026-07-16改訂：タブ順を基本情報→写真→採寸→説明文に変更、商品一覧から開いた際のデフォルトタブも基本情報に統一**）
   - 4a. 基本情報：**商品番号（SKU）・商品名の編集欄を追加**（他の商品と重複するSKUに変更した場合は警告表示）／対象（メンズ/レディース/ユニセックス）・カテゴリ・サイズ・コンディション。**カテゴリは2026-07-20改訂：自由入力ではなくSquareの`ListCatalog`（`types=CATEGORY`）で取得した既存カテゴリのみをプルダウンから選択する方式に変更**（自由入力フォールバックなし）
   - 4b. 写真管理：非公開R2へのアップロード、正面／追加写真の管理、Square CatalogImageへの添付・削除同期。正面写真はSquareのプライマリ画像とする
   - 4c. 採寸：正面写真アップロード後の自動計測結果・手動修正（マット・マーカーはこの画面でのみ使用）
...
- 情報入力とSquare登録が最初の1ステップに凝縮され、写真・採寸・コンディション・対象等は「後で追記できるサブフロー」に切り出された。
- 一覧画面に状態バッジを追加し、「登録は速いが詳細が空のまま」という状態を可視化・追跡できるようにした（写真・採寸が入力されないまま放置されるリスクへの対策）。**2026-07-16改訂：このバッジ・フィルタ・統計は一旦Square登録状況（登録済み／未登録）のみを表す形に統一し、詳細入力状況のトラッキングは作り直すまで一旦外した。**

### 商品詳細編集画面（画面4）のUI設計

モバイル幅を基準にしたモックアップを作成済み（本セッションのArtifact）。構成は以下の通り。

画面全体のアクセントカラーは店舗テーマに合わせた`#EA3325`とし、主要ボタン、リンク、
チェックボックス、選択中タブへ共通適用する。ライト・ダーク表示とも文字色は白とする。

- **ヘッダー**：戻る導線／商品名・SKU・価格（**2026-07-16改訂：「SKU」ラベルは付けず番号のみ表示**）／状態バッジ（Square未登録／Square未反映／Square同期済み／Square側で削除済み）／登録済み商品の「Squareの最新情報を取得」
- **タブ（4つ、単一画面内で切り替え。2026-07-16改訂：並び順を基本情報→写真→採寸→説明文に変更。商品一覧から遷移した際の初期表示タブも基本情報に統一）**
  - **基本情報**：**商品番号（SKU）・商品名の編集欄**（先頭）。他の商品と重複するSKUに変更すると赤字で警告表示。対象（メンズ／レディース／ユニセックスのセレクト）、カテゴリ（**2026-07-20改訂：Squareの`ListCatalog`APIから取得した既存カテゴリのプルダウン選択式。自由入力は廃止**。取得結果はセッション中1回だけWorker経由でフェッチしキャッシュ。親カテゴリを持つ場合は「親 > 子」の形でラベル表示）・表記サイズ（テキスト入力）、コンディション（セレクトの先頭に「未設定（後で設定）」を用意）。
  - **写真**：正面写真（採寸トリガーとして単独スロット、1枚のみ）と、追加写真（固定カテゴリなしのギャラリー、0〜何枚でも自由）の2セクション。背面・タグ・襟元・ダメージのような固定ロールは設けない（撮る/撮らないが商品次第なため）。0枚でも保存可能な旨を明記。
  - **採寸**：正面写真の有無で分岐。ある場合は自動計測結果（着丈・身幅・肩幅・袖丈）と信頼度、「再計測」ボタンを表示。数値タップで手動修正できる導線。
  - **説明文**：テンプレートから自動生成した説明文プレビュー。商品名・SKUは常に現在値から組み立てるため、基本情報タブでの変更が即座に反映される。未設定項目は行ごと省略される旨を明記。コンディションは「■コンディション：{選択中のランク}」に続けてS〜D全ランクの説明文を常に列挙する（ECサイトで説明文だけを見た人にもランクの意味が伝わるようにするため）。
- **フッター（常設）**：「下書き保存」（プライマリ・左／Supabaseだけを更新）／未登録時は「Squareに登録」、登録済み時は「Squareを更新」（セカンダリ・右／Supabase保存後にSquareへ反映）。処理内容がボタン名だけで判別できるようにする。

### 実装方針：`docs/mvp_prototype.html` の扱い

`docs/mvp_prototype.html`は単一HTMLファイル＋バニラJSの「動く仕様書」（画面遷移は`.screen`のshow/hideとlocalStorage、マーカー検出は自前のARライブラリ実装、バックエンド無し・単一端末前提）。新しい画面構成（商品一覧⇄クイック登録⇄商品詳細編集、Supabase・Cloudflare Workers連携、複数端末共有）とは前提が大きく異なるため、**このファイルをそのまま本番アプリへ改修していく方針ではない**。
...
**2026-07-16実装：マーカー無しの簡易検出（`packages/measure`）**
現行のWeb先行フローには専用マット・ArUcoマーカーが無いため、`mvp_prototype.html`のマーカー検出・ホモグラフィ補正はそのままでは使えない。代わりに`packages/measure`には**背景差分によるシルエット検出**を実装した：
- 画像の外周を背景色サンプルとして扱い、そこから色距離が離れた画素を前景（服）とみなして外接矩形（bounds）を求める（`computeForegroundBounds`）
- `DEFAULT_MEASURE_POINTS`（7点）が前提とする基準領域（`REFERENCE_BOUNDS`）から、検出した実際の矩形へ各点を比例配置し直す（`placeLandmarksFromBounds`）
- 検出できなかった場合（無地でない背景・服が縁まではみ出す等）は`DEFAULT_MEASURE_POINTS`にフォールバックし、UI側は「検出できませんでした・中央に仮配置」と正直に表示する
- あくまで目安の位置合わせであり、実寸への換算はマーカーによる縮尺基準が無いため比例計算のまま（絶対値の精度は保証しない）。マーカー＋ホモグラフィによる正確な実寸補正は、専用マットを導入する場合の将来課題として残す。
---
...
├── supabase/
│   └── migrations/            # 初期スキーマ＋Square画像ID追加、テスト運用向け公開RLS
...
**役割分担の原則**：
- `apps/web`：画面（商品一覧／クイック登録／商品詳細編集）とSupabaseへの通常のCRUD（テスト中はanonキーで公開アクセス）
...
- `apps/web`はSquare APIを直接叩かず、`apps/worker`の`POST /api/items/:id/register-to-square`、`PATCH /api/items/:id/square`、`POST /api/items/:id/sync-from-square`、`POST /api/items/:id/photos`、`DELETE /api/items/:id/photos/:photoId`、`POST /api/items/:id/photos/sync-to-square`、`GET /api/square/categories`を利用する。SquareトークンとR2バインディングはWorker内に閉じる。
...
- 元要件：[square_connect_plan.md](./docs/square_connect_plan.md)（0章に2026-07-16の方針変更まとめあり）
- 動くプロトタイプ：`docs/mvp_prototype.html`
