...
1. ログイン（スタッフ認証）
2. **商品一覧（ホーム）** — 各行に状態バッジを表示。「詳細未設定あり」とだけ出しても何が足りないか分からないため、**未設定の項目名を具体的に表示**する（例：「写真・採寸など未設定」、最大2件＋残りは「など」で省略）。後回しにした項目が放置されないよう可視化する、という元の目的はそのまま。＋新規登録ボタン。あわせて商品管理システム的な要素として、**統計サマリー**（**2026-07-16改訂：総数／Square登録済み／Square未登録の3件のみ**。詳細入力状況の集計は今後作り直すため一旦除外）、**検索・絞り込み**（商品名・カテゴリ・SKUでの検索、状態でのフィルタ）、**並べ替え**（登録が新しい/古い順、価格が高い/安い順）を備える。
...
- **ヘッダー**：戻る導線／商品名・SKU・価格／状態バッジ（未設定項目を具体的に列挙。例：「写真・採寸など未設定」）
