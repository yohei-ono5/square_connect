...
1. ログイン（スタッフ認証）
2. **下書き一覧（ホーム）** — 各行に状態バッジ（Square下書き／詳細未設定あり／公開済み）を表示。後回しにした項目が放置されないよう、未設定項目の有無を一覧上で可視化する。＋新規登録ボタン。あわせて商品管理システム的な要素として、**統計サマリー**（総数／詳細未設定あり／詳細入力済みの件数カード）、**検索・絞り込み**（商品名・ブランド・SKUでの検索、状態でのフィルタ）、**並べ替え**（登録が新しい/古い順、価格が高い/安い順）を備える。
