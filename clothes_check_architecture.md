...
1. ログイン（スタッフ認証）
2. **商品一覧（ホーム）** — 各行に状態バッジを表示。「詳細未設定あり」とだけ出しても何が足りないか分からないため、**未設定の項目名を具体的に表示**する（例：「写真・採寸など未設定」、最大2件＋残りは「など」で省略）。後回しにした項目が放置されないよう可視化する、という元の目的はそのまま。＋新規登録ボタン。あわせて商品管理システム的な要素として、**統計サマリー**（総数／詳細未設定あり／詳細入力済み、および**Square登録済み／未登録**の件数カード）、**検索・絞り込み**（商品名・カテゴリ・SKUでの検索、状態でのフィルタ）、**並べ替え**（登録が新しい/古い順、価格が高い/安い順）を備える。
...
4. **商品詳細編集**（タブ構成、いつでも中断・再開可）
   ...
   - 4c. 基本情報：対象（メンズ/レディース/ユニセックス）・カテゴリ・サイズ・コンディション
...
- **タブ（4つ、単一画面内で切り替え）**
  ...
  - **基本情報**：対象（メンズ／レディース／ユニセックスのセレクト）、カテゴリ・表記サイズ（テキスト入力）、コンディション（セレクトの先頭に「未設定（後で設定）」を用意）。
  - **説明文**：テンプレートから自動生成した説明文プレビュー。未設定項目は行ごと省略される旨を明記。コンディションは「■コンディション：{選択中のランク}」に続けてS〜D全ランクの説明文を常に列挙する（ECサイトで説明文だけを見た人にもランクの意味が伝わるようにするため）。
